...
#define	CH1CONF		(0x140>>2)
#define	CH1STAT		(0x144>>2)
#define CH1CTRL		(0x148>>2)
#define	  TX1		(0x14c>>2)
#define	  RX1		(0x150>>2)
...
static enum hrtimer_restart Device1TimerInterrupt1(struct hrtimer *pTimer)
{
	...
	MCSPI1[CH1CONF] |= 1<<20;
	MCSPI1[CH1CTRL] |= 1;
	rx_data = MCSPI1[CH1STAT];
//	printk("Reg ch1stat = %x\n", rx_data);

//	udelay(1000);
	while(!(MCSPI1[CH1STAT] & 0x2))
		;

	MCSPI1[TX1] = (ULONG)tx_data++;
	if(tx_data > 0xFFF0)
		tx_data = 0;
...
static void Device1Init(void)
{
	...
	MCSPI1[MODULCTRL] &= ~(0x1);

	MCSPI1[CH1CONF] = 0x103C9;




      	Device1Time  =  ktime_set(0, 1000000 * 10);
